...
# LOCAL_SRC = Path("/data0/Sougata/Dataset/IndicVoices_R")  # your hf download dir (root)
# OUT_ROOT  = Path("/data0/Sougata/Dataset/IndicVoices_R/All_Languages_Extracted")
# SPLITS    = ["train", "test"]
# # ======================================
...
#     roots = [
#         LOCAL_SRC,
#         Path(os.environ.get("HF_DATASETS_CACHE", Path.home() / ".cache" / "huggingface" / "datasets")),
#     ]
#     for root in roots:
...
#     try:
#         ds = load_dataset(REPO_ID, lang, split=split)  # no trust_remote_code
...
# ========= CONFIG (edit these) =========
REPO_ID   = "ai4bharat/indicvoices_r"
LOCAL_SRC = Path("/data0/Sougata/Dataset/IndicVoices_r_cpy")  # your hf download dir (root)
OUT_ROOT  = Path("/data0/Sougata/Dataset/IndicVoices_r_cpy/All_Languages_Extracted")
SPLITS    = ["train", "test"]
# ======================================
...
def try_resolve_path(basename: str) -> Path | None:
    ...
    roots = [
        LOCAL_SRC,
        Path(os.environ.get("HF_DATASETS_CACHE", "/data0/Sougata/Dataset/IndicVoices_r_cpy")),
    ]
    for root in roots:
        if root and root.exists():
            hits = list(root.rglob(Path(basename).name))
...
import os, csv, shutil, glob, sys
import os, pathlib
os.environ["HF_HOME"] = "/data0/IndicVoices_r_cpy/hf"
os.environ["HF_DATASETS_CACHE"] = "/data0/IndicVoices_r_cpy/hf/datasets"
os.environ["HF_HUB_CACHE"] = "/data0/IndicVoices_r_cpy/hf/hub"
os.environ["TMPDIR"] = "/data0/IndicVoices_r_cpy/tmp"
...
def load_from_hub_or_none(lang: str, split: str):
    try:
        ...
        ds = ds.cast_column("audio", Audio(decode=False))
        ds.set_format("python")#!/usr/bin/env python3
import os, csv, shutil, glob, sys
from pathlib import Path
from tqdm import tqdm
from datasets import load_dataset, get_dataset_config_names, Audio, Dataset
